...
<?php
    ...
    function dataMember() {
        global $conn;
        $member = mysqli_query($conn, "SELECT * FROM perpustakaan");
    ...
    function tambahMember($id,$nama,$nomor,$alamat,$tgldaftar,$tglbayar) {
        global $conn;
        $sql = "INSERT INTO perpustakaan VALUES ('$id', '$nama','$nomor', '$alamat', '$tgldaftar', '$tglbayar')";
        $result = mysqli_query($conn, $sql);
    ...
    function hapusMember($id_member){
        global $conn;
        $sql = "DELETE FROM perpustakaan WHERE id_member = $id_member";
        $result = mysqli_query($conn, $sql);
        if ($result) {
            header('location:Member.php');
        }
